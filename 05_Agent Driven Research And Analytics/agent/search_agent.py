...
def create_search_agent(llm, members, working_directory):
    ...
    tools = [
        create_document, # Create and save a text document in Markdown format.
        read_document, # Reading the document
        collect_data, # Collect data from a CSV file.
        wikipedia, 
        google_search, # Perform a Google search based on the given query and return the top 5 results.
        scrape_webpages_with_fallback # Attempt to scrape webpages using FireCrawl, falling back to WebBaseLoader if unsuccessful.
    ] + load_tools(["arxiv"],)
