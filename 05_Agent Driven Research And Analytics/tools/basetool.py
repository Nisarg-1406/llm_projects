...
from load_cfg import WORKING_DIRECTORY,CONDA_ENV
...
def get_platform_specific_command(command: str) -> tuple:
    ...
    """
    conda_commands = [
        # Directly activate the Conda environment without sourcing conda.sh
        f"eval \"$(conda shell.bash hook)\"",  # Initialize Conda for the current shell
        f"conda activate {CONDA_ENV}",  # Activate the environment
