...
def hypothesis_router(state: State) -> NodeType:
    ...
    - If hypothesis_content is empty, it goes to the Hypothesis node.
    - If hypothesis_content is filled, it goes to the Process node.
    """
...
def QualityReview_router(state: State) -> NodeType:
    ...
    if (last_message and 'REVISION' in str(last_message.content)) or state.get("needs_revision", False):
        ...
        result = revision_routes.get(previous_node, "Process") # If previous node not found, return to "process" node
...
def process_router(state: State) -> ProcessNodeType:
    ...
    Here we check the state['process_decision'] and here we check the 'next' key which points out to {"Coder", "Search", "Visualization", "Report"} 
    state['process_decision'] consist of "next" node and the "task" for that node.
    """
    ...
    try:
        if isinstance(process_decision, AIMessage):
            ...
            try:
                ...
                # print("DECISION DICT: ", decision_dict)
